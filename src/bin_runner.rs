...
fn make_map(seed: u64) -> State {
    ...
    while positions.len() < target && tries < 10000 {
        ...
        if cells[idx(x,y,WIDTH)].terrain != PLAINS { continue; }
        ...
        if positions.iter().any(|&(px,py)| cells[idx(px,py,WIDTH)].region_id == region) { continue; }
        positions.push((x,y));
    }

    let mut towns = Vec::new();
    for (i,(x,y)) in positions.iter().enumerate() {
        towns.push(Town { id: i as i32, x:*x, y:*y, desired: vec![] });
    }
...
fn send_initial<W: Write>(w: &mut W, state: &State, my_id: i32) {
    ...
    for t in &state.towns {
        let desired = if t.desired.is_empty() {
            ...
            t.desired.iter().map(|x| x.to_string()).collect::<Vec<_>>().join(",")
        };
...
fn send_turn<W: Write>(w: &mut W, state: &State, my_score: i32, foe_score: i32) {
    ...
    for c in &state.cells {
        let active = if c.active.is_empty() {
            ...
            c.active.iter().map(|(a,b)| format!("{a}-{b}")).collect::<Vec<_>>().join(",")
        };
        writeln!(w, "{} {} {} {}", c.track_owner, c.instability, c.inked as i32, active).unwrap();
    }
    w.flush().unwrap();
}
...
fn expand_autoplace(state: &State, a: &Action) -> Vec<Action> {
    match *a {
        Action::Auto(x1,y1,x2,y2) => {
            ...
            path.into_iter()
                .filter(|&(x,y)| town_at(state,x,y).is_none() &&
...
fn action_place_cost(state: &State, x: usize, y: usize) -> Option<i32> {
    if x >= state.width || y >= state.height { return None; }
    if town_at(state,x,y).is_some() { return None; }
    let c = &state.cells[idx(x,y,state.width)];
    if c.track_owner != FREE || c.inked { return None; }
...
fn apply_simultaneous(state: &mut State, a0: &[Action], a1: &[Action]) -> (i32,i32) {
    ...
    for ((x,y), owners) in requests {
        let owner = if owners.len() >= 2 { NEUTRAL } else { owners[0] as i32 };
...
fn main() {
    let bot = std::env::args().nth(1).unwrap_or_else(|| {
        if cfg!(windows) { "target/debug/bot.exe".into() } else { "target/debug/bot".into() }
    });
